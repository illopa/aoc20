...
let data = fs.readFileSync('esempio.txt').toString().replace(/\r/g,"").split("\n");
...
can = can.reduce( (r,c) => r.concat( c[1].map( (a) => [c[0],a]) )  ,[]);

let gby = _.mapValues( _.groupBy(can, (a) => a[1] ), (l) => l.map( (b) => b[0]));
gby = _.mapValues(gby, (l) => _.intersection(...l));
